...
function ProfileDropdown({ user, logout }: any) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-900 shadow-lg rounded-lg overflow-hidden border border-slate-100 dark:border-slate-800">
          <Link href={user?.role === 'worker' ? '/worker/dashboard' : '/customer'} className="block px-4 py-2 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300">
            Dashboard
          </Link>
          <Link href="/settings" className="block px-4 py-2 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300">
            Settings
          </Link>
          <button onClick={logout} className="block w-full text-left px-4 py-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20">
            Logout
          </button>
        </div>
      )}
